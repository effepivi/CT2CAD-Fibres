...
def simulateSinogram():
    ...
    # Add gaussian noise
    diff = reference_normalised_projections - normalised_projections;
    stddev = diff.std();    
    normalised_projections += np.random.normal(0.0, stddev);
...
def fitnessFunction(x):
    global raw_projections_in_MeV, t, lsf_param, scale;

    kernel=lsf(t*41,lsf_param[0],lsf_param[1],lsf_param[2],lsf_param[3],lsf_param[4],lsf_param[5]);
    kernel/=kernel.sum();

    temp = copy.deepcopy(raw_projections_in_MeV);
    
    # Apply the LSF line by line
    for z in range(temp.shape[0]):
        for y in range(temp.shape[1]):
            temp[z][y] = np.convolve(raw_projections_in_MeV[z][y], kernel, mode='same');

    raw_projections_in_keV = temp / gvxr.getUnitOfEnergy("keV");
    normalised_projections = flatFieldCorrection(raw_projections_in_keV);
    
    
    normalised_projections = np.add(normalised_projections, np.random.normal(x[0] * scale[0], (x[1] + 0.5)  * scale[1], normalised_projections.shape));  
    simulated_sinogram = computeSinogramFromFlatField(normalised_projections);
...
# Load the matrix
setMatrix([-0.03238304, -0.01171571, 0.25231507, 0.24146212, 0.2274979]);

# Load the cores and fibres
fibre_radius = 49.55; # um
core_radius = 7.69; # um
setFibres(centroid_set);

raw_projections_in_MeV = tomographyAcquisition();




lsf_param = [569.568077709076,	-38.3079251552199,	-0.512792514950253,	0.356348840589841,	-73261364.9022948,	-0.138302181237248];

scale = [0.000001, 0.0125];
...
es = cma.CMAEvolutionStrategy(x0, 0.09, opts);
es.optimize(fitnessFunction);
elapsed_time = time.time() - start_time
print("NOISE",elapsed_time);


noise_param = copy.deepcopy(es.result.xbest);

noise_param[0] = noise_param[0] * scale[0];
noise_param[1] = (noise_param[1] + 0.5)  * scale[1];

np.savetxt(output_directory + "/noise_param.dat", noise_param, header='centre,sigma');
...
old_shape = normalised_projections.shape;
normalised_projections.shape = [old_shape[1],old_shape[0],old_shape[2]];
...
sitk.WriteImage(volume, output_directory + "/normalised_projections_without_noise.mha", useCompression=True);
normalised_projections.shape = old_shape;


# Add gaussian noise
normalised_projections = np.add(normalised_projections, np.random.normal(noise_param[0], noise_param[1], normalised_projections.shape));

normalised_projections.shape = [old_shape[1],old_shape[0],old_shape[2]];

volume = sitk.GetImageFromArray(normalised_projections);
volume.SetSpacing([g_pixel_spacing_in_mm, g_angular_step, g_pixel_spacing_in_mm]);
sitk.WriteImage(volume, output_directory + "/normalised_projections_with_noise.mha", useCompression=True);
normalised_projections.shape = old_shape;

simulated_sinogram = computeSinogramFromFlatField(normalised_projections);

simulated_sinogram.shape = (simulated_sinogram.size // simulated_sinogram.shape[2], simulated_sinogram.shape[2]);
reconstruction_CT_LSF = iradon(simulated_sinogram.T, theta=g_theta, circle=True);

volume = sitk.GetImageFromArray(reconstruction_CT_LSF);
volume.SetSpacing([g_pixel_spacing_in_mm, g_pixel_spacing_in_mm, g_pixel_spacing_in_mm]);
sitk.WriteImage(volume, output_directory + "/reconstruction_CT_LSF.mha", useCompression=True);



print("noise:", noise_param);
normalised_reconstruction_CT_LSF = (reconstruction_CT_LSF - reconstruction_CT_LSF.mean()) / reconstruction_CT_LSF.std();
ZNCC_CT = np.mean(np.multiply(normalised_reconstruction_CT_LSF.flatten(), normalised_reference_CT.flatten()));
print("noise CT ZNCC:", ZNCC_CT);




comp_equalized = compare_images(normalised_reference_CT, normalised_reconstruction_CT_LSF, method='checkerboard');
...
comp_equalized[comp_equalized < -0.5] = -0.5;
comp_equalized[comp_equalized > 1.25] = 1.25;

comp_equalized -= np.min(comp_equalized);
comp_equalized /= np.max(comp_equalized);

io.imsave(output_directory + "/comparison.png", comp_equalized);
