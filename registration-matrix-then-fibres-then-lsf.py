...
def fitnessFunction(x):
    global raw_projections_in_MeV, t, lsf_param;
    ...
    kernel=lsf(t*41,y[0],y[1],y[2],y[3],y[4],y[5]);
    kernel/=kernel.sum();
...
lsf_param = [601.873, 54.9359, -3.58452, 0.469614, 6.32561e+09, 1.0];
x0 = [0, 0, 0, 0, 0, 0];
...
lsf_param[0] *= es.result.xbest[0];
lsf_param[1] *= es.result.xbest[1];
lsf_param[2] *= es.result.xbest[2];
lsf_param[3] *= es.result.xbest[3];
lsf_param[4] *= es.result.xbest[4];
lsf_param[5] *= es.result.xbest[5];

np.savetxt(output_directory + "/lsf_param.dat", lsf_param, header='a,b,c,d,e,f');
